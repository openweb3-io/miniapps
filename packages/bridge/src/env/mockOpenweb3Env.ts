...
/**
 * Mocks a Openweb3 application environment.
 * @param launchParamsRaw - launch parameters presented as a string or query parameters.
 */
export function mockOpenweb3Env(launchParamsRaw: LaunchParams | string): void {
  ...
  const proxyPostEvent = (window as any).Openweb3WebviewProxy?.postEvent;
  (window as any).Openweb3WebviewProxy = {
  ...
  logInfo('Environment was mocked by the mockOpenweb3Env function');
}