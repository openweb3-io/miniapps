...
To call Openweb3 Mini Apps methods, developers should use the `postEvent` function:
...
environment. It identifies the Openweb3 app type and selects the appropriate flow.

### `request`

The `request` function should be used when a developer needs to call a Openweb3 Mini Apps method and
...
If the Openweb3 Mini Apps method accepts parameters, they should be passed in the `params` property
...
The `postEvent` option allows a developer to override the method used to call the Openweb3 Mini Apps
method.
...
The `postEvent` function does not check if the specified method is supported by the current Openweb3
