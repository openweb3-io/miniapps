...
from [@openweb3-apps/sdk](../packages/openweb3-apps-sdk/2-x).
...
authentication. The fact is that the data generated by the native Openweb3 application is signed
with the secret key of the Openweb3 bot, after which the generated signature is placed next to the
parameters themselves.

Thus, knowing the secret key of the Openweb3 bot, the developer has the opportunity to verify the
...
3. Create HMAC-SHA256 using key `WebAppData` and apply it to the Openweb3 Bot
...
- For Node: [@openweb3-apps/init-data-node](../packages/openweb3-apps-init-data-node)
...
`WebAppData` string literal value and Openweb3 Bot token.
...
<tbody>
  ...
  <tr>
    ...
    <td>
      ...
      <a href="https://core.openweb3.org/bots/api#answerwebappquery">answerWebAppQuery</a>.
    </td>
  </tr>
...
| is_premium               | `boolean` | _Optional_. Has the user purchased Openweb3 Premium.                                                                                                             |
