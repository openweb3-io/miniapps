import { createCbCollector, addEventListener, type If, type IsNever } from '@openweb3-apps/toolkit';
import { Signal, signal } from '@openweb3-apps/signals';
import { miniAppsMessage, type MiniAppsMessage } from '@openweb3-apps/transformers';
...
function defineListeners(onEvent: (event: LastEvent) => void): () => void {
  ...
  const [, cleanup] = createCbCollector(
    ...
    // The desktop version of Openweb3 is sometimes not sending the "viewport_changed"
    // event. For example, when the Main Button is shown. That's why we should
    // add our own listener to make sure viewport information is always fresh.
    // Issue: https://github.com/openweb3-io/miniapps/issues/10
    ...
    // Add listener, which handles events sent from the Openweb3 web application and also events
    ...
    addEventListener(w, 'message', (event) => {
      ...
      try {
        ...
        logError(
          [
            `An error occurred processing the "${eventType}" event from the Openweb3 application.`,
            'Please, file an issue here:',
            'https://github.com/openweb3-io/miniapps/issues/new/choose',
          ].join('\n'),
          message,
          cause,
        );
